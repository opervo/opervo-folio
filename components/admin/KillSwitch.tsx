...
export default function KillSwitch({ initialEnabled, lastTripReason }: Props) {
  ...
  return (
    <>
      <div style={{ background: enabled ? '#FFFFFF' : '#FEE7DA', border: `2px solid ${enabled ? '#E8E4DE' : '#F5620F'}`, borderRadius: 12, padding: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 700, fontSize: 20, textTransform: 'uppercase' }}>
              Kill Switch
            </div>
            <div style={{ color: '#6B6B6B', fontSize: 14, marginTop: 4 }}>
              {enabled
                ? 'Automation is running. All tasks pull through the orchestrator.'
                : tripped
                ? `Auto-tripped. Reason: ${lastTripReason}`
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'grid', placeItems: 'center', zIndex: 100 }}>
          <div style={{ background: '#FFFFFF', borderRadius: 12, padding: 32, maxWidth: 480, width: '90%' }}>
            <h2 style={{ fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 700, fontSize: 24, margin: 0, textTransform: 'uppercase' }}>
              {action} automation?
            </h2>
            {tripped && isResume && (
              <div style={{ background: '#FEE7DA', border: '1px solid #F5620F', borderRadius: 8, padding: 12, fontSize: 13, marginTop: 12 }}>
                ...
                <br />
                Type a clear acknowledgement of what was fixed. This is recorded.
              </div>
            )}
